...
export class GenericSelectComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {

  // Dynamic Component Data
  @Input('controlName') controlName: FormControl;
  @Input('required') required: boolean = false;
  @Input('placeholder') placeholder: string = 'Select';
  @Input('data') data: Generic;

  // Store Data
  public dataList: Generic[] = [];
  public filteredDataList: ReplaySubject<Generic[]> = new ReplaySubject<Generic[]>(1);
  searchDataList: Generic[] = [];
        ...
        switchMap(search => {
          if (!this.searchCtrl.value.trim()) {
            ...
            if (this.selectCtrl.value) {
              ...
              const allGenerics = [...this.searchDataList, ...this.dataList]
              const selectedData = allGenerics.filter(m => {
          ...
          return this.genericService.getAllGeneric(filterData, search);
        })
      )
      .subscribe({
        next: res => {
  ...
  private getAllGenericList() {
    ...
    this.genericService.getAllGeneric(filter, null)
      .subscribe({
        next: res => {
