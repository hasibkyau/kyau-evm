...
export class UnitSelectComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {

  // Dynamic Component Data
  @Output() onSelect = new EventEmitter<Unit>();
  @Input('controlName') controlName: FormControl;
  @Input('required') required: boolean = false;
  @Input('placeholder') placeholder: string = 'Select';
  @Input('data') data: Unit;

  // Store Data
  public dataList: Unit[] = [];
  public filteredDataList: ReplaySubject<Unit[]> = new ReplaySubject<Unit[]>(1);
  searchDataList: Unit[] = [];
        ...
        switchMap(search => {
          if (!this.searchCtrl.value.trim()) {
            ...
            if (this.selectCtrl.value) {
              ...
              const allUnits = [...this.searchDataList, ...this.dataList]
              const selectedData = allUnits.filter(m => {
          ...
          return this.unitService.getAllUnits(filterData, search);
        })
      )
      .subscribe({
        next: res => {
  ...
  private getAllUnitList() {
    ...
    this.unitService.getAllUnits(filter, null)
      .subscribe({
        next: res => {
