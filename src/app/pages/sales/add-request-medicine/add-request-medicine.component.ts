...
import {RequestMedicineService} from '../../../services/common/request-medicine.service';
import {RequestMedicine} from '../../../interfaces/common/request-medicine';

@Component({
  selector: 'app-add-request-medicine',
  templateUrl: './add-request-medicine.component.html',
  styleUrls: ['./add-request-medicine.component.scss'],
  providers: [PricePipe, VariationNormalizePipe],
})
export class AddRequestMedicineComponent implements OnInit, OnDestroy {
  // Static Data
  ...
  onSubmit() {
    ...
    const orderData: any = {
      ...
      requestMedicine: this.requestMedicine?._id,

      requestMedicineType: true,
      user: this.requestMedicine?.user,
  ...
  private getRequestMedicineById() {
    this.spinnerService.show();
    this.subDataTwo = this.requestMedicineService.getRequestMedicineById(this.id).subscribe(
      (res) => {
        this.spinnerService.hide();
        if (res.success) {
          this.requestMedicine = res.data;
          console.log("getRequestMedicineById",this.requestMedicine)
          // if (this.order.user) {
