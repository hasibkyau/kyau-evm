...
export class AddCategoryComponent implements OnInit {
  ...
  private initDataForm() {
    this.dataForm = this.fb.group({
      ...
      status: ['publish'],
      discountAmount: [null],
      isHomed: [this.hasAccess[0].value],
    });
  }
  ...
  private setFormValue() {
    ...
    if (this.category && this.category.bannerImage) {
      this.pickedBannerImage = this.category.bannerImage;
    }
  }
      ...
      if (dialogResult) {
        if (dialogResult.data && dialogResult.data.length > 0) {
          ...
          if (type === 'mobileImage') {
            ...
            console.log(this.pickedBannerImage);
          }else {
