...
export class SearchResultComponent implements OnInit, OnChanges, OnDestroy {
  ...
  onSubmit() {
    ...
    const mData = {
      ...
      bookingTime: this.dataForm.value.ticketType === 'Booked' ? (this.dataForm.value.bookingTime ? Number(this.dataForm.value.bookingTime) : null) : null,
      expiredIn: null,
      bus: {
  ...
  onShow(event: MouseEvent | any) {
    ...
    if (!this.id) {
      ...
      this.router.navigate([], {queryParams: {trip: null}, queryParamsHandling: 'merge'}).then();
    }
  ...
  onSelectSeat(data: Seat) {
    if (data.status === 'Available') {
      let findIndex = this.selectedTrip?.seats.findIndex((s: any) => s._id === data?._id);
      ...
      if (data.status === 'Selected' && this.carts.find(f => f.seat === data._id)) {
        let findIndex = this.selectedTrip?.seats.findIndex((s: any) => s._id === data?._id);
        this.selectedTrip.seats[findIndex].seatAnimation = true;
        const cartData: Cart = {
          trip: this.selectedTrip._id,
  ...
  private addCart(data: Cart, index: number, type: 'add' | 'remove') {
    this.subDataOne = this.cartService.addCart(data)
      .subscribe({
        next: res => {
          this.selectedTrip.seats[index].seatAnimation = false;
          if (res.success) {
            this.uiService.success(res.message);
            this.selectedTrip.seats[index].status = type === 'add' ? 'Selected' : 'Available';
            if (this.mode === 'edit' && type === 'remove') {
              const fIndex = this.selectedSeats.findIndex(f => f._id === data.seat);
  ...
  private getCartByTrip() {
    ...
    this.subDataTwo = this.cartService.getCartByTrip(data)
      .subscribe({
        next: res => {
          this.carts = res.data;
          const seats: Seat[] = []
          if (this.carts.length) {
            this.carts.forEach(cart => {
              const fSeat = this.selectedTrip.seats.find(f => f._id === cart.seat);
  ...
  private bookTrip(data: any) {
    this.subDataThree = this.tripService.bookTrip(data)
      .subscribe({
        next: res => {
  ...
  private updateBookedTrip(data: any) {
    this.subDataThree = this.tripService.updateBookedTrip(this.ticket._id, data)
      .subscribe({
        next: res => {
          this.isLoading = false;
          if (res.success) {
